<template>
  <el-container v-loading="serviceLoading">
    ...
    <el-dialog title="包编辑" :visible.sync="moduleVisible" class="my-dialog" @close="closeModule">
      <div style="width: 80%">
        <el-form :model="moduleInfo" :rules="moduleRules" label-width="150px" label-position="right" ref="moduleForm">
          ...
          <el-form-item label="包id" prop="zipModule">
  ...
  export default {
    data () {
      return {
        ...
        isNew ({row}) {
          if (row.isNew === '1') {
            return { 'background-color': '#EBF6FD' }
          }
        },
        getKey (row) {
          return row.id
          ...
          if (!valid) {
            ...
            if (type === 'service') {
              ...
              let data = {
                ...
                serviceModuleId: this.serviceInfo.id,
                isAdd: '0'
              }
              if (this.handleServiceWay === 'add') {
                data.isAdd = '1'
              }
              ...
              let data = {
                ...
                zipModule: this.moduleInfo.zipModule,
                isAdd: '0'
              }
              if (this.handleModuleWay === 'add') {
                data.isAdd = '1'
              }
