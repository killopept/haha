    ...
    <el-container class="page-table">
      ...
      <el-main>
        ...
        <el-dialog title="版本升级详细规则" :visible.sync="versionUpgradListVisible">
          <el-table class="version-detail-table"
                    :data="versionUpgradList"
                    v-loading="loading3"
                    border
                    width="100%">
            <el-table-column  prop="version"  label="版本号"></el-table-column>
            <el-table-column  prop="upgradeType"  label="升级类型"></el-table-column>
          </el-table>
...
export default {
  ...
  watch: {
    addDialogFormVisible (val) {
      if (val === false) {
        this.$refs['addUpgradeForm'].resetFields()
      }
    }
  },
  methods: {
    ...
    // 添加发布包-平台切换
    platformChange () {
      this.$refs['addUpgradeForm'].clearValidate()
    ...
    // 详情页面获取
    getVersionDetailPage (currentPg, currentPgNum) {
    ...
    detailRow (row) {
      // 获取版本更新状态信息
      this.versionUpgradListPage.currentPage = ''
      this.versionUpgradListPage.currentPage = 1
      this.versionUpgradListPage.total = ''
      this.currentDetailRow.platform = row.platform === 'IOS' ? '2' : '1'
      this.currentDetailRow.packageInfoId = row.id
      this.loading3 = true
      this.versionUpgradList = []
      this.getVersionDetailPage(1, 10)
    },
    // 切换详情页页码
    handleCurrentListChange (val) {
      this.versionUpgradListPage.currentPage = val
      this.loading3 = true
      this.getVersionDetailPage(val, 10)
    },
    // 修改
