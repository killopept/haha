      ...
      <el-header  class="border">
        <router-link to="/main/upgradeManage">
          ...
          <span> 
            ...
            <span class="head-sp">{{fatherData.platform==='1'?'Android':fatherData.platform==='2'?'IOS':fatherData.platform}}</span>
          </span>
        </router-link>
      </el-header>
    ...
    if (this.fatherData.selected === '1') {
      ...
      getUpgradeTaskDetail(sendItem).then(res => {
        ...
        if (upgradeTaskDetail.upgradeType === '1') {
          ...
          this.pgradeTaskForm.upgradeType = '1'
        } else if (upgradeTaskDetail.upgradeType === '2') {
          this.pgradeTaskForm.upgradeType = '1'
          this.pgradeTaskForm.upgradeType = '2'
        }
        // 发布模式
