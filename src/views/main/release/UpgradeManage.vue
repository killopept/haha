...
export default {
  ...
  methods: {
    ...
    getUpgradeTaskFunc (data) {
      getUpgradeTask(data).then(res => {
        ...
        this.versionTaskList.forEach(res => {
          ...
          res.upgradeType = res.upgradeType === '0' ? '忽略更新' : res.upgradeType === '2' ? '提示更新' : '强制更新'
          res.confirmDelete = false
        })
        // console.log(this.versionTaskList)
      })
    },
    // 显示第几页
    ...
    detailRow (row) {
      // 获取版本更新状态信息
      var detailItem = {
        platform: row.platform === 'IOS' ? '2' : '1',
        packageInfoId: row.id
      }
      getVersionDetail(detailItem).then(res => {
        this.versionUpgradList = res.data.body.versionUpgradList
